...
export interface Page {
   ...
   *
   * When all step_definitions combined have not finished during the specified
...
export interface AndroidInput {
   ...
   * @param steps The number of step_definitions in the drag. Each step takes 5 milliseconds to complete.
   */
  drag(from: {
    x: number;
   ...
   * @param steps The number of step_definitions for each segment. Each step takes 5 milliseconds to complete, so 100 step_definitions means half a second
   * per each segment.
   */
  swipe(from: {
    x: number;
